...
(function () {
  ...
  function post(path, body) {
    return fetch(path, {
      ...
      if (!r.ok) {
        el('status').textContent = 'That didn\\u2019t work (HTTP ' + r.status + ') \\u2014 the login process may have ended. Try "Restart Claude login".';
        throw new Error('http ' + r.status);
      }
      return r;
    }).catch(function (e) {
      if (e && /Failed to fetch|NetworkError|Load failed/.test(String(e))) {
        el('status').textContent = 'Portal unreachable \\u2014 it may have shut down. Check the terminal on the server.';
      }
      throw e;
    });
  }
