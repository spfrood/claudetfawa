...
class LoginDriver extends EventEmitter {
  ...
  start() {
    ...
    this.idleCheck = setInterval(() => {
      // While the CLI is deliberately waiting on the human (OAuth in another
      // tab can easily take longer than any sane pty timeout), don't idle-kill
      // it — the portal's own inactivity shutdown still bounds its lifetime.
      const waitingOnHuman = this.status.state === 'url-ready' || this.status.state === 'code-error';
      if (this.alive && !this.finished && !waitingOnHuman && Date.now() - this.lastActivity > this.ptyTimeoutMs) {
  ...
  onData(chunk) {
    ...
    if (this.codeSubmitted && !this.finished && !this.finishing) {
  ...
  writeCode(code) {
    ...
    // Real codes are long enough to trip the TUI's paste-burst handling, which
    // swallows a newline glued to the paste — the field fills but never
    // submits. Send Enter as its own keystroke once the paste has settled.
  ...
  checkCredentials() {
    if (this.finishing || this.finished) return true;
  ...
  // Credentials landing is NOT the end: on a first-run install the CLI is
  // mid-onboarding, and killing it here leaves onboarding incomplete — the
  // next plain `claude` start then re-runs onboarding INCLUDING a fresh OAuth
  // login, ignoring the valid credentials on disk (verified against 2.1.200).
  // Press Enter through the remaining screens until the REPL appears, so the
  // CLI records onboarding as done, then kill it.
  finishOnboarding() {
    this.finishing = true;
    clearInterval(this.credsPoll);
    this.setStatus('finishing', 'Credentials verified — letting the CLI finish its first-run setup…');
    let lastLen = this.raw.length;
    let stable = 0;
    let presses = 0;
    this.finishTimer = setInterval(() => {
      const flat = stripAnsi(this.raw).replace(/\s+/g, '').toLowerCase();
      if (flat.slice(-3000).includes('?forshortcuts') || presses >= 8 || !this.alive) {
        return this.completeSuccess();
      }
      if (this.raw.length === lastLen) {
        stable += 1;
      } else {
        stable = 0;
        lastLen = this.raw.length;
      }
      if (stable >= 2) {
        presses += 1;
        stable = -2; // give the screen time to react before pressing again
        this.pty.write('\r');
        this.touch();
      }
    }, 700);
    this.finishDeadline = setTimeout(() => this.completeSuccess(), 30000);
  }
  ...
  cleanupTimers() {
    ...
    clearTimeout(this.finishDeadline);
  }
